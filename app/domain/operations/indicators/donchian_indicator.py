...
class DonchianIndicator(BaseIndicator):
    ...
    def calculate_signals(self, df: pd.DataFrame, param_str: str) -> pd.DataFrame:
        ...
        df['donchian_high'] = ta.MAX(close, timeperiod=params.window)
        df['donchian_low'] = ta.MIN(close, timeperiod=params.window)
        ...
        signal_col_id = f"{StrategyLibEnum.DONCHIAN.name.lower()}_signal"
