...
async def train_lgbm_endpoint(lgb_train_settings_dict: dict, background_tasks: BackgroundTasks): 
    service = LgbTrainService()
    background_tasks.add_task(service.train_yahoo_data, lgb_train_settings_dict)
    return {"status": "training started"}


@router.post("/optimize-lgb-training")
async def optimize_train_lgb_endpoint(lgb_train_settings_dict: dict, background_tasks: BackgroundTasks):

    background_tasks.add_task(optimize_lgb_training, lgb_train_settings_dict)
    return {"status": "optimize_strategy started"}


@router.post("/test-lgb")
async def test_lgb_endpoint(lgb_test_settings_dict: dict, background_tasks: BackgroundTasks): 
    test_service = AlpacaTestService()
    background_tasks.add_task(test_service.test_lgb_model, lgb_test_settings_dict)
